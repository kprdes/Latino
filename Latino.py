...
def guardar_respuestas(respuestas):
    credentials = service_account.Credentials.from_service_account_info(
        st.secrets["gcp_service_account"],  # Añade las credenciales como secreto en Streamlit Sharing
        scopes=["https://www.googleapis.com/auth/cloud-platform"],
    )

    # Intenta cargar el archivo CSV existente desde Google Cloud Storage
    try:
        with gcsfs.GCSFileSystem(project="tu-proyecto", token=credentials) as fs:
            with fs.open("gs://tu-bucket/respuestas_encuesta.csv", "rb") as f:
    ...
    with gcsfs.GCSFileSystem(project="tu-proyecto", token=credentials) as fs:
        with fs.open("gs://tu-bucket/respuestas_encuesta.csv", "w") as f:
